...
<div class="container">
	<div class="row">
		<div class="col-md-8 col-lg-8" id="contentContainer">
			<c:forEach items="${cartAdverts}" var="advert">
					...
					<div class="panel-body">
						<div class="row">
							<div class="col-lg-10 col-md-10">
								...
								<img height="100" width="150" src="/resources/images/${advert.imageName}"><br>
								<a href="/advert/${advert.id}">Перейти к объявлению</a><br>
								<c:if test="${currentUser.id == 0}">
									<strong>Чтобы удалить из корзины, пожалуйста, войдите в систему</strong><br>
								</c:if>
								<c:if test="${currentUser.id != 0}">
									<a class="btn btn-primary" href="/removeFromCart/${advert.id}" role="button">Удалить из корзины</a><br>
								</c:if>
								<hr>
							</div>
		...
		<div class="col-md-4 col-lg-4" id="rightBarContainer">
			<c:if test="${currentUser.id != 0}">
				<strong>Итого:</strong> ${paymentAmount} BYN<br>
				<a class="btn btn-primary" href="/makeOrder" role="button">Оформить заказ</a><br>
			</c:if>
		</div>
	</div>
</div>
